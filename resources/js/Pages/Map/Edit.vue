    ...
    <div class="w-full">
            ...
            <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
              ...
              <div class="mt-1 sm:mt-0 sm:col-span-2">
                <div class="max-w-lg flex rounded-md shadow-sm">
                  <input id="name" v-model="form.name" v-on:keydown.enter="$refs.submit.click()"
                    :class="{ 'border-red-500 text-red-900 placeholder-red-300 focus:border-red-500 focus:shadow-outline-red': $page.errors.name}"
                    class="flex-1 form-input block w-full rounded-none rounded-md transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                </div>
                <p class="mt-2 text-xs text-red-600" v-show="$page.errors.name" v-for="error in $page.errors.name">
                  {{ error }}</p>
              </div>
            </div>
              ...
              <div class="mt-1 sm:mt-0 sm:col-span-2">
                <div class="max-w-lg flex rounded-md shadow-sm">
                  <textarea v-model="form.description" id="description" rows="3"
                    :class="{ 'border-red-500 text-red-900 placeholder-red-300 focus:border-red-500 focus:shadow-outline-red': $page.errors.description}"
                    class="form-textarea block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5"></textarea>
                </div>
                <p class="mt-2 text-xs text-red-600" v-show="$page.errors.description"
                  v-for="(error, index) in $page.errors.description" :key="index">{{ error }}</p>
      ...
      <div class="mt-8 border-t border-gray-200 pt-5">
        <div class="flex justify-end">
          ...
          <span class="ml-3 inline-flex rounded-md shadow-sm">
            <button :disabled="loading" @click="updateMap" type="button" ref="submit"
