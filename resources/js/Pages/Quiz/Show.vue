  ...
  <layout :title="'Quiz : ' + map.name" :quiz-mode="true">
    ...
    <div class="w-full h-screen bg-gray-400">
        ...
        <div class="flex flex-col md:flex-row h-full">
          ...
          <div class="text-center px-1 pt-1 md:px-2 md:py-2 md:m-2 flex flex-col justify-center md:w-3/12 lg:w-3/12 xl:w-2/12">
            <div class="overflow-hidden">
              <ul>
                <li @click="selectAnswer(answer.id)" v-for="(answer, index) in answers" :key="answer.id" class="rounded md:rounded-md mb-2" :class="{'bg-blue-300': checkAnswer(answer.id) == 'checking', 'bg-green-300': correctAnswer == answer.id, 'bg-red-300': checkAnswer(answer.id) == false, 'bg-white': checkAnswer(answer.id) == null}">
                  <div class="block focus:outline-none transition duration-150 ease-in-out select-none" :class="{'focus:bg-gray-100 hover:bg-gray-100': selectedAnswer == null}">
                    <div class="flex items-center px-2 py-2 md:py-4 md:px-6">
                      <div class="min-w-0 flex-1 flex items-center">
                        <div class="min-w-0 flex-1 px-4">
                          <div>
                            <div class="text-sm leading-5 font-medium text-indigo-600 truncate">{{ answer.callout }}</div>
                          </div>
                        </div>
                      </div>
                    </div>
