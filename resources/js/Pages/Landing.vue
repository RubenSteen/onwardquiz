...
<div v-if="$page.flash.warning" class="fixed bottom-0 inset-x-0 px-4 pb-4 sm:inset-0 sm:flex sm:items-center sm:justify-center z-50">
  <!--
  ...
  <div class="relative bg-white rounded-lg px-4 pt-5 pb-4 overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
    <div class="hidden sm:block absolute top-0 right-0 pt-4 pr-4">
      <button type="button" class="text-gray-400 hover:text-gray-500 focus:outline-none focus:text-gray-500 transition ease-in-out duration-150" aria-label="Close" @click="$page.flash.warning = null">
    ...
    <div class="sm:flex sm:items-start">
      ...
      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline">
          {{ $page.flash.warning.title }}
        </h3>
        <div class="mt-2">
          <p class="text-sm leading-5 text-gray-500">
            {{ $page.flash.warning.message }}
          </p>
        </div>
      </div>
    </div>
      ...
      <nav class="relative flex items-center justify-between sm:h-10 md:justify-center">
        ...
        <div class="hidden md:absolute md:flex md:items-center md:justify-end md:inset-y-0 md:right-0">
          <span v-if="$page.auth.check === true" class="inline-flex rounded-md shadow">
            <a :href="$route('user.profile')" class="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100 hover:text-indigo-700 focus:outline-none focus:bg-gray-100 focus:text-indigo-700 transition duration-150 ease-in-out">
              My profile
            </a>
          </span> 
          <span class="inline-flex rounded-md shadow ml-3">
            <a v-if="$page.auth.check === false" :href="$route('login')" class="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100 hover:text-indigo-700 focus:outline-none focus:bg-gray-100 focus:text-indigo-700 transition duration-150 ease-in-out">
              Log in
            </a>
            <button v-else @click="$inertia.post($route('logout.post'))" type="button" class="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100 hover:text-indigo-700 focus:outline-none focus:bg-gray-100 focus:text-indigo-700 transition duration-150 ease-in-out">
              Log out
            </button>
          </span>
        </div>
    ...
    <div x-show="open" style="display: none;" class="absolute top-0 inset-x-0 p-2 md:hidden">
      <div class="rounded-lg shadow-md transition transform origin-top-right" x-show="open" x-transition:enter="duration-150 ease-out" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="duration-100 ease-in" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95">
        <div class="rounded-lg bg-white shadow-xs overflow-hidden">
          ...
          <div>
            <inertia-link v-if="$page.auth.check === false" :href="$route('login')" class="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100 hover:text-indigo-700 focus:outline-none focus:bg-gray-100 focus:text-indigo-700 transition duration-150 ease-in-out">
              Log in
            </inertia-link>
            <button v-else @click="$inertia.post($route('logout.post'))" type="button" class="block w-full px-5 py-3 text-center font-medium text-indigo-600 bg-gray-50 hover:bg-gray-100 hover:text-indigo-700 focus:outline-none focus:bg-gray-100 focus:text-indigo-700 transition duration-150 ease-in-out">
              Log in
      ...
      <div class="text-center">
        ...
        <div class="mt-5 max-w-md mx-auto sm:flex sm:justify-center md:mt-8">
          <div class="rounded-md shadow">
            <a v-if="$page.auth.check === false" :href="$route('login')" class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:shadow-outline-indigo transition duration-150 ease-in-out md:py-4 md:text-lg md:px-10">
              Get started
            </a>
            <inertia-link v-else :href="$route('quiz.index')" class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:shadow-outline-indigo transition duration-150 ease-in-out md:py-4 md:text-lg md:px-10">
              Get started
            </inertia-link>
          </div>
    ...
    <div class="relative mt-12 lg:mt-24 lg:grid lg:grid-cols-2 lg:gap-8 lg:items-center">
      <div class="relative">
        ...
        <p class="mt-3 text-lg leading-7 text-gray-500">
          This website stays a fun project next to playing onward, we always seek ways to improve so most features are still in development/planning but here is a list that you can already do if you register!
        </p>
        ...
        <ul class="mt-10">
          <li>
            <div class="flex">
              ...
              <div class="ml-4">
                <h5 class="text-lg leading-6 font-medium text-gray-900">Teams</h5>
                <p class="mt-2 text-base leading-6 text-gray-500">
                  Create and manage teams, invite your fellow onward players to join your team! and keep track on who you rather not have standing next to you on certain maps when they fail at the callouts!
                  <br>
                  <small>Results tracker still has to be implemented</small>
                </p>
              </div>
            </div>
          </li>
          <li class="mt-10">
            <div class="flex">
              ...
              <div class="ml-4">
                <h5 class="text-lg leading-6 font-medium text-gray-900">Callout quiz</h5>
                <p class="mt-2 text-base leading-6 text-gray-500">
                  Take a quiz on <b>most</b> onward competetive VRML league maps, and see directly which you know and where to improve!
                </p>
              </div>
            </div>
          </li>
    ...
    <div class="relative mt-12 sm:mt-16 lg:mt-24">
      <div class="lg:grid lg:grid-flow-row-dense lg:grid-cols-2 lg:gap-8 lg:items-center">
        <div class="lg:col-start-2">
          ...
          <p class="mt-3 text-lg leading-7 text-gray-500">
            These core features are either planned or being currently worked on, we are always open for suggestions.
          </p>
          ...
          <ul class="mt-10">
            <li>
              <div class="flex">
                ...
                <div class="ml-4">
                  <h5 class="text-lg leading-6 font-medium text-gray-900">Email quiz notifications</h5>
                  <p class="mt-2 text-base leading-6 text-gray-500">
                    Set a reminder to take a quiz every week, or as a team leader make sure your members stay up to date and schedule a quiz.
                  </p>
                </div>
              </div>
            </li>
            <li class="mt-10">
              <div class="flex">
                ...
                <div class="ml-4">
                  <h5 class="text-lg leading-6 font-medium text-gray-900">Quiz tracker</h5>
                  <p class="mt-2 text-base leading-6 text-gray-500">
                    Have insight how many times you took a quiz and the results, or as a team leader stay up to date on which members deserve the whip for slacking.
                  </p>
                </div>
              </div>
            </li>
  ...
  export default {
    ...
    watch: {
      title: {
        immediate: true,
        handler(title) {
          document.title = 'Welcome - ' + this.$page.app.name
        },
      }, // End title
    }, // End watch
    // mounted(){
