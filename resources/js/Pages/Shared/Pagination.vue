<template>
  ...
  <div v-if="links.length > 3" class="border-t border-gray-200 px-4 sm:px-0 flex items-center justify-between">

    <div class="w-0 flex-1 flex" v-if="links[[links.length-1]].label === 'Next'">
      <inertia-link :href="links[0].url"
    ...
    <div class="hidden md:flex">
      <inertia-link :href="link.url" v-for="(link, key) in links" :key="key"
        v-if="!['Previous', 'Next'].includes(link.label)"
        :class="{'border-indigo-500 text-indigo-600 focus:border-indigo-700 focus:text-indigo-800': link.active, 'border-transparent text-gray-500 focus:border-gray-400 focus:text-gray-700 ': !link.active}"
    ...
    <div class="w-0 flex-1 flex justify-end" v-if="links[0].label === 'Previous'">
      <inertia-link :href="links[[links.length-1]].url"
