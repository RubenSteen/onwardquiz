  ...
  <layout title="Users">
    ...
    <div class="py-4">
        ...
        <ul class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <li v-for="(user, index) in users" :key="user.id" class="col-span-1 flex flex-col text-center bg-white rounded-lg shadow">
            <div class="flex-1 flex flex-col p-8">
              <img v-if="user.avatar" class="w-32 h-32 flex-shrink-0 mx-auto bg-black rounded-full" :src="user.avatar" :alt="'Discord avatar of '+ user.getFullUsername">
              <span v-else class="w-32 h-32 flex-shrink-0 mx-auto inline-block rounded-full overflow-hidden bg-gray-100">
              ...
              <dl class="mt-1 flex-grow flex flex-col justify-between">
                ...
                <dd class="text-gray-500 text-sm leading-5">Last activity : <i>{{ user.last_activity }}</i></dd>
                <dt class="sr-only">Roles</dt>
                <dd class="mt-3">
                  <span v-show="user.isSuperAdmin" class="px-2 py-1 text-teal-800 text-xs leading-4 font-medium bg-teal-100 rounded-full">Super Admin</span>
                  <span v-show="user.isEditor" class="px-2 py-1 text-teal-800 text-xs leading-4 font-medium bg-teal-100 rounded-full">Editor</span>
                  <span v-show="user.isEditor == false && user.isSuperAdmin == false" class="px-2 py-1 text-gray-800 text-xs leading-4 font-medium bg-gray-100 rounded-full">No roles</span>
                </dd>
                <dt class="sr-only">Status</dt>
                <dd class="mt-3">
                  <span v-show="user.isConfirmed" class="px-2 py-1 text-blue-800 text-xs leading-4 font-medium bg-blue-100 rounded-full">Confirmed</span>
                  <span v-show="user.isBanned" class="px-2 py-1 text-red-800 text-xs leading-4 font-medium bg-red-100 rounded-full">Banned</span>
                  <span v-show="user.isConfirmed == false && user.isBanned == false" class="px-2 py-1 text-gray-800 text-xs leading-4 font-medium bg-gray-100 rounded-full">No statuses</span>
                </dd>
              </dl>
            </div>
        ...
        <div v-show="selectedUser != null" class="fixed bottom-0 inset-x-0 px-4 pb-6 sm:inset-0 sm:p-0 sm:flex sm:items-center sm:justify-center">
        <!--
        ...
        -->
        <div v-if="selectedUser != null" class="bg-white rounded-lg px-4 pt-5 pb-4 overflow-hidden shadow-xl transform transition-all sm:max-w-lg w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
          <div>
              
            <div class="mx-auto flex items-center justify-center">
              <img v-if="selectedUser.avatar" class="h-12 w-12 flex-shrink-0 mx-auto bg-black rounded-full" :src="selectedUser.avatar" :alt="'Discord avatar of '+ selectedUser.getFullUsername">
              <span v-else class="h-12 w-12 flex-shrink-0 mx-auto inline-block rounded-full overflow-hidden bg-gray-100">
                <svg class="h-full w-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
              </span>
          ...
          <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
            ...
            <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:col-start-1">
              <button @click="selectedUser = null" type="button" class="inline-flex justify-center w-full rounded-md border border-gray-300 px-4 py-2 bg-white text-base leading-6 font-medium text-gray-700 shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">
                Cancel
              </button>
            </span>
          </div>
